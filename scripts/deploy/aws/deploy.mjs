...
async function main() {
  ...
  execFileSync("tar", ["--no-xattrs", "-czf", archive, "--exclude=node_modules", "--exclude=dist", "--exclude=.env*", "--exclude=*.env", "--exclude=*.pem", "--exclude=*.key", "--exclude=keys", "--exclude=online-keys", "--exclude=secret-bindings.json", "--exclude=*.tsbuildinfo", "--exclude=test-results", "--exclude=*.test.ts", "--exclude=*.test.mjs", "package.json", "package-lock.json", "tsconfig.base.json", "src/api", "src/worker", "src/db", "src/domain", "src/hedera-native", "src/hedera-ats", "src/contracts/artifacts/contracts/ReceivablePoolRegistry.sol/ReceivablePoolRegistry.json", "src/contracts/artifacts/contracts/SnapshotPayoutAdapter.sol/SnapshotPayoutAdapter.json", "docs/evidence/testnet-evidence.json", "scripts/deploy/aws"], { stdio: "inherit", env: { ...process.env, COPYFILE_DISABLE: "1" }, timeout: 60000 });
